...
export const processConcurrentWithPool = async <T, R>(
  ...
  try {
    ...
    log.info(
      `Starting concurrent processing: ${items.length} items with concurrency ${concurrency}`,
    );
      ...
      limit(async () => {
        ...
        try {
          ...
          if (duration > 5000) {
            // Log slow operations
            log.warn(
              `Slow operation detected: item ${index} took ${duration}ms`,
            );
          }
    ...
    log.info(
      `Concurrent processing completed: ${successful} successful, ${failed} failed out of ${items.length} total`,
    );
